...
$e^{x_i \beta } + u_i \not \approx e^{x_i \hat \beta } \Sigma^n_{i=1} e^{\hat u_i} \partial u_i$
...
\noindent At least one coefficient of exogenous variable, intuitively, has important contributions to this model. Except for the p-value for intercept, each p-value for every coefficient is below 0.01. The p-value for intercept equals 0.881, so this coefficient is not different from zero. A chance of receiving this rating 3 point is, *cerberus paribus*, 50%. 
