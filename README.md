...
<p align = 'justify'>  The lasso regularization is a popular method that proves to be successful in data science. The function of this regularization is the residual sum of squares plus the penalty. The penalty is a lambda multiplied by sum of absolute value of coefficient parameters. This function is robust to the outlier but it is not differentiable due to piecewise function inderivative (Boehmke, 2021). This algorithm the ability to identify unimportant predictors due to sparsity (predictors are set to 0).</p>

<p align = 'justify'>  The Bayes optimal feature selection is an iterative process by balancing its needs of exploration and exploitation depending on the probabilistic model and acquisition function. This algorithm can solve complex derivative-free problems. There is an objective function that possesses ture shape hidden in the model and this shape is not completely observable that can otherwse be expensive to compute. The surrogate function is a probablistic model that is built to exploit what is known. The acquisition function is to calculate a vector of hyperparameters that likely yield higher local maximum of objective function. One of these studies shows that this approach has the ability to reduce the number of features and yield higher model performance, competing with many state-of-the-art methods (Ahmed et al., 2015).</p>
